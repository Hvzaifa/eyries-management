...
import { updateEmdRound } from '../actions/emd';
import { formatEmdNumberInput } from '@/lib/format';
import type { EmdRoundView } from '@/lib/pnrs';

export default function EditRoundButton({ round, licenses }: { pnrId: string, round: EmdRoundView, licenses: { id: string, name: string }[] }) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append('round_id', round.id);

    startTransition(async () => {
      const res = await updateEmdRound(formData);
      if (res?.error) {
        setError(res.error);
      } else {
        setIsOpen(false);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl border border-stone-200 w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-5 py-4 border-b border-stone-100 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-stone-900">Edit Round {round.roundNumber}</h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" name="round_issuance_date" required defaultValue={round.issuanceDate || ''} className={inputCls} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-stone-600 mb-1.5">
                    Issuance Time
                  </label>
                  <input type="time" name="round_issuance_time" defaultValue={round.issuanceTime || ''} className={inputCls} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" name="round_payment_pct" required defaultValue={round.paymentPct || ''} className={inputCls} />
                </div>
              </div>
              ...
              <div>
                ...
                <input type="number" name="round_emd_amount" required defaultValue={round.emdAmount} className={inputCls} />
              </div>
              ...
              <div>
                ...
                <select name="round_license_id" defaultValue={round.licenseId ?? ''} className={`${inputCls} cursor-pointer`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="date" name="round_deadline_date" required defaultValue={round.deadlineDate || ''} className={inputCls} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-stone-600 mb-1.5">
                    Deadline Time
                  </label>
                  <input type="time" name="round_deadline_time" defaultValue={round.deadlineTime || ''} className={inputCls} />
                </div>
              </div>
